...
export default function Finance({ user }: { user: User }) {
  ...
  // mês (via handleLaunchVariableCost, que cria uma transação de verdade) — por isso não
  // entram nessa soma automática, senão contariam mesmo sem terem sido lançados.
  const activeFixedCostsTotal = useMemo(
    () => fixedCosts.filter(c => c.active && c.costType === 'fixed').reduce((sum, c) => sum + c.amount, 0),
    [fixedCosts]
  );
  ...
  const handleLaunchVariableCost = async (cost: FixedCost) => {
    ...
    try {
      ...
      await updateDoc(doc(db, 'fixedCosts', cost.id!), { lastLaunchedMonth: currentMonthKey });
      showToast('Custo variável lançado neste mês');
    } catch (err) {
      showToast('Erro ao lançar custo', 'error');
    }
  ...
  const handleSaveFixedCost = async () => {
    ...
    try {
      if (editingFixedCost) {
        ...
        showToast(fixedCostType === 'fixed' ? 'Custo fixo adicionado' : 'Custo variável adicionado');
      }
      resetFixedCostForm();
    } catch (err) {
      ...
      {financeView === 'balancete' ? (
        ...
        <div className="max-w-[1800px] mx-auto space-y-8">
          ...
          <div className="p-6 bg-red-50 rounded-2xl border border-red-100 text-sm text-red-500 font-medium">
            Total fixo (automático): R$ {activeFixedCostsTotal.toLocaleString('pt-BR', { minimumFractionDigits: 2 })} / mês
          </div>

          {(isAddingFixedCost || editingFixedCost) && (
            <div className="p-8 bg-[#FDFBF9] border border-[#F5F2F0] rounded-[32px] space-y-6">
              <div>
                <label className="block text-[10px] font-bold text-[#9CA3AF] uppercase tracking-widest mb-2 ml-1">Tipo de Custo</label>
                <div className="flex gap-3">
                  <button
                    type="button"
                    onClick={() => setFixedCostType('fixed')}
                    className={`flex-1 py-4 rounded-2xl text-xs font-bold uppercase tracking-widest transition-all ${fixedCostType === 'fixed' ? 'bg-[#EADFD4] text-white shadow-md' : 'bg-white text-[#9CA3AF] border border-[#F5F2F0]'}`}
                  >
                    Fixo (repete sozinho)
                  </button>
                  <button
                    type="button"
                    onClick={() => setFixedCostType('variable')}
                    className={`flex-1 py-4 rounded-2xl text-xs font-bold uppercase tracking-widest transition-all ${fixedCostType === 'variable' ? 'bg-[#EADFD4] text-white shadow-md' : 'bg-white text-[#9CA3AF] border border-[#F5F2F0]'}`}
                  >
                    Variável (lança todo mês)
                  </button>
                </div>
              </div>
              ...
              <div className="flex gap-4">
                ...
                >
                  {editingFixedCost ? 'Salvar Alterações' : 'Adicionar Custo'}
                </button>
              </div>
            </div>
          )}
          ...
          <div className="space-y-3">
            {fixedCosts.map(cost => {
              const isVariable = cost.costType === 'variable';
              const launchedThisMonth = cost.lastLaunchedMonth === currentMonthKey;
              return (
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <div className="flex items-center gap-2">
                      ...
                      <span className={`text-[8px] font-bold uppercase tracking-widest px-2 py-0.5 rounded-full ${isVariable ? 'bg-[#FDF3E7] text-[#C9A15A]' : 'bg-[#F0F7F0] text-[#8BA888]'}`}>
                    ...
                    <p className="text-[10px] text-[#9CA3AF] uppercase font-bold tracking-widest mt-0.5">
                      ...
                      {isVariable && cost.active && (launchedThisMonth ? ' — já lançado este mês' : ' — ainda não lançado este mês')}
                    </p>
                  </div>
                </div>
                <div className="flex items-center gap-1">
                  {isVariable && cost.active && (
                    <button
                      onClick={() => handleLaunchVariableCost(cost)}
                      disabled={launchedThisMonth || launchingCostId === cost.id}
                      className={`px-4 py-2 rounded-xl text-[9px] font-bold uppercase tracking-widest transition-all mr-2 ${launchedThisMonth ? 'bg-[#F0F7F0] text-[#8BA888]' : 'bg-[#EADFD4] text-white hover:bg-[#DFCFBF]'} disabled:opacity-60`}
                    >
                      {launchedThisMonth ? '✓ Lançado' : launchingCostId === cost.id ? 'Lançando...' : 'Lançar Este Mês'}
                    </button>
                  )}
                  <button
                    onClick={() => { setEditingFixedCost(cost); setFixedCostDescription(cost.description); setFixedCostAmount(String(cost.amount)); setFixedCostType(cost.costType || 'fixed'); setIsAddingFixedCost(false); }}
  ...
  const handlePrint = () => {
    ...
    if (period === 'mensal') {
      ...
      rows = `
        ...
        <p style="margin-top: 12px;"><strong>Origem das Entradas</strong></p>
        ${incomeRows}
        <p style="margin-top: 12px;"><strong>Origem das Saídas</strong></p>
        ${expenseRows}
      `;
      totalsHtml = `
            ...
            <div className="bg-white p-8 rounded-[32px] border border-[#F5F2F0] shadow-sm">
              ...
              <div className="space-y-3">
                ...
                {monthlyData.incomeByCategory.length === 0 && (
                  <p className="text-xs text-[#9CA3AF] italic">Nenhuma entrada neste mês.</p>
                )}
              </div>
            </div>
            <div className="bg-white p-8 rounded-[32px] border border-[#F5F2F0] shadow-sm">
              <p className="text-[10px] font-bold text-[#9CA3AF] uppercase tracking-widest mb-5">Origem das Saídas</p>
              <div className="space-y-3">
                {monthlyData.expenseByCategory.map(c => (
                  <div key={c.category} className="flex items-center justify-between text-sm">
                    <span className="text-[#4A433D]">{c.category}</span>
                    <span className="text-red-400 font-medium">R$ {c.total.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</span>
                  </div>
                ))}
                {monthlyData.expenseByCategory.length === 0 && (
                  <p className="text-xs text-[#9CA3AF] italic">Nenhuma saída neste mês.</p>
                )}
              </div>
            </div>
          </div>
