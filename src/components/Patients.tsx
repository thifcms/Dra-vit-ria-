...
function PatientDetail({ user, patient, onBack, onReturnToSchedule }: { user: User, patient: Patient, onBack: () => void, onReturnToSchedule?: () => void }) {
  ...
  const handleLaunchToFinance = async (procedureName: string, quantity: number = 1) => {
    ...
    try {
      ...
      const updatedTxIds = { ...(anamnesis.launchedProcedureTransactionIds || {}), [procedureName]: docRef.id };
      setAnamnesis({ ...anamnesis, launchedProcedures: updatedLaunched, launchedProcedureTransactionIds: updatedTxIds });
  ...
  const handleUnlaunchFromFinance = async (procedureName: string) => {
    ...
    try {
      ...
      const updatedLaunched = (anamnesis.launchedProcedures || []).filter(n => n !== procedureName);
      const updatedTxIds = { ...(anamnesis.launchedProcedureTransactionIds || {}) };
