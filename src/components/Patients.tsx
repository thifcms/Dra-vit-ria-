...
function PatientDetail({ user, patient, onBack, onReturnToSchedule }: { user: User, patient: Patient, onBack: () => void, onReturnToSchedule?: () => void }) {
  ...
  const normalizeAnamnesis = (a: any) => {
    const defaultAnamnesis = {
      ...
      plannedProcedureQuantities: a?.plannedProcedureQuantities || {},
      launchedProcedures: a?.launchedProcedures || [],
      launchedProcedureTransactionIds: a?.launchedProcedureTransactionIds || {},
    };
  ...
  // Lança o procedimento marcado na Conduta direto no financeiro — automático, assim
  // que o procedimento é marcado (não precisa de um segundo clique). Guarda o ID da
  // transação criada pra poder remover automaticamente se o procedimento for
  // desmarcado depois. Quantidade multiplica o valor lançado.
  const handleLaunchToFinance = async (procedureName: string, quantity: number = 1) => {
    ...
    try {
      const docRef = await addDoc(collection(db, 'transactions'), {
        userId: user.uid,
        amount: proc.price * quantity,
        type: 'income',
        date: new Date(),
        category: procedureName,
        description: `${procedureName}${quantity > 1 ? ` (${quantity}x)` : ''}${substanceName ? ` (${substanceName})` : ''} — ${patient.name}`,
        patientId: patient.id,
        autoGenerated: true,
      });
      // Usa a forma funcional (prev => ...) em vez de espalhar a variável "anamnesis"
      // direto — essa função é assíncrona (o addDoc acima demora um pouquinho), e
      // nesse meio tempo o clique que chamou essa função JÁ tinha atualizado
      // plannedProcedures. Espalhar a "anamnesis" capturada no momento em que essa
      // função começou (antes daquela atualização) sobrescrevia esse ponto — o
      // procedimento aparecia marcado por um instante e "desmarcava sozinho" assim que
      // essa parte assíncrona terminava. A forma funcional sempre parte do estado mais
      // recente de verdade, não de uma versão presa no tempo.
      let updatedLaunched: string[] = [];
      let updatedTxIds: Record<string, string> = {};
      setAnamnesis(prev => {
        updatedLaunched = [...(prev.launchedProcedures || []), procedureName];
        updatedTxIds = { ...(prev.launchedProcedureTransactionIds || {}), [procedureName]: docRef.id };
        return { ...prev, launchedProcedures: updatedLaunched, launchedProcedureTransactionIds: updatedTxIds };
      });
      await updateDoc(doc(db, 'patients', patient.id!), {
        'anamnesis.launchedProcedures': updatedLaunched,
        'anamnesis.launchedProcedureTransactionIds': updatedTxIds,
      }).catch(() => {});
    } catch (err) {
  ...
  // Remove o lançamento automático quando o procedimento é desmarcado — reverso exato
  // do lançamento acima.
  const handleUnlaunchFromFinance = async (procedureName: string) => {
    const txId = anamnesis.launchedProcedureTransactionIds?.[procedureName];
    try {
      if (txId) await deleteDoc(doc(db, 'transactions', txId));
      let updatedLaunched: string[] = [];
      let updatedTxIds: Record<string, string> = {};
      setAnamnesis(prev => {
        updatedLaunched = (prev.launchedProcedures || []).filter(n => n !== procedureName);
        updatedTxIds = { ...(prev.launchedProcedureTransactionIds || {}) };
        delete updatedTxIds[procedureName];
        return { ...prev, launchedProcedures: updatedLaunched, launchedProcedureTransactionIds: updatedTxIds };
      });
      await updateDoc(doc(db, 'patients', patient.id!), {
        'anamnesis.launchedProcedures': updatedLaunched,
      ...
      showToast('Erro ao retirar do financeiro', 'error');
    }
  };

  // Atualiza o valor já lançado quando a quantidade de um procedimento já lançado muda
  const handleUpdateLaunchedQuantity = async (procedureName: string, quantity: number) => {
    const proc = procedures.find(p => p.name === procedureName);
    const txId = anamnesis.launchedProcedureTransactionIds?.[procedureName];
    if (!proc || !txId) return;
    const substanceName = anamnesis.plannedSubstances?.[procedureName];
  ...
  // Assinatura remota de orçamento — reforço sempre ativo (independente de estar na
  // aba Orçamento no momento): antes esse mesclador só existia dentro do
  // BudgetGenerator, que só é montado quando a aba Orçamento está aberta — se o
  // paciente assinasse enquanto o profissional estivesse em outra aba, a mesclagem
  // nunca acontecia, e o histórico ficava sempre com valor zero. O de dentro do
  // BudgetGenerator continua existindo (cuida do travamento visual da tela quando ela
  // está aberta), mas o campo "mergedIntoRecord" evita processar duas vezes o mesmo
  // pedido, então não há risco de duplicar a entrada no histórico.
  useEffect(() => {
    const q = query(
      collection(db, 'signRequests'),
      where('patientId', '==', patient.id),
    ...
    const unsubscribe = onSnapshot(q, async (snap) => {
      try {
        ...
        if (toMerge.length === 0) return;
        const freshSnap = await getDoc(doc(db, 'patients', patient.id!));
        const freshBudgetHistory = freshSnap.exists() ? ((freshSnap.data() as Patient).budgetHistory || []) : (patient.budgetHistory || []);
        const newEntries = [];
        for (const d of toMerge) {
          const data = d.data();
          const budgetNumber = await getNextBudgetNumber(db);
          newEntries.push({
            id: crypto.randomUUID(),
            budgetNumber,
            date: new Date().toISOString(),
            ...
            validityDays: data.budgetValidityDays || '15',
        ...
        await updateDoc(doc(db, 'patients', patient.id!), {
          budgetHistory: [...freshBudgetHistory, ...newEntries],
        });
        await Promise.all(toMerge.map(d => updateDoc(doc(db, 'signRequests', d.id), { mergedIntoRecord: true })));
        showToast('Orçamento assinado remotamente — guardado no prontuário');
      } catch (err: any) {
        console.error('Erro ao mesclar assinatura remota de orçamento:', err);
        showToast(`Não foi possível salvar o orçamento assinado: ${err?.code || err?.message || 'erro desconhecido'}`, 'error');
      }
    });
    return () => unsubscribe();
  }, [patient.id]);
  ...
  const handlePrintBudgetHistory = (entry: NonNullable<Patient['budgetHistory']>[number]) => {
    ...
    const bodyHtml = `
      ...
      <div class="box">
        <div class="box-label">Orçamento${entry.budgetNumber ? ` nº ${entry.budgetNumber}` : ''}</div>
        ${itemsHtml}
  ...
  const [sendingEvolutionSignIndex, setSendingEvolutionSignIndex] = useState<number | null>(null);

  // Assinatura remota de um registro de evolução específico — mesmo padrão da anamnese:
  // gera link, manda por WhatsApp/e-mail, e um watcher sempre ativo (perto do de
  // anamnese, abaixo) libera o registro sozinho quando o paciente assina.
  const handleSendEvolutionForSignature = async (index: number, via: 'whatsapp' | 'email') => {
    ...
    setSendingEvolutionSignIndex(index);
    try {
      const ownerId = await getClinicOwnerId(db).catch(() => user.uid);
      const content = `Evolução — ${draft.procedure}\n\n${draft.notes || ''}`;
      const requestData = {
        userId: user.uid,
        patientId: patient.id,
        patientName: patient.name,
        patientCpf: patient.cpf || '',
        templateId: 'evolution',
        templateTitle: 'Registro de Evolução',
      ...
      const docRef = await addDoc(collection(db, 'signRequests'), requestData);
      const link = remoteSignLink(docRef.id);
      const message = `Olá, ${patient.name}! Segue o link pra revisar e assinar seu registro de evolução:\n${link}`;
      if (via === 'whatsapp') {
        openWhatsApp(sentTo, message);
      } else {
        window.open(genericEmailLink(sentTo, 'Assinatura: Registro de Evolução', message), '_blank');
      }
      showToast(`Link gerado — confirme o envio no ${via === 'whatsapp' ? 'WhatsApp' : 'e-mail'}`);
    } catch (err) {
      showToast('Erro ao gerar o link', 'error');
    }
    setSendingEvolutionSignIndex(null);
  };

  // Detecta a assinatura remota de evolução e libera o registro sozinho — mesmo
  // princípio do watcher de anamnese, mas usando evolutionIndex pra saber qual
  // rascunho específico foi assinado (podem existir vários rascunhos ao mesmo tempo)
  useEffect(() => {
    const q = query(
      collection(db, 'signRequests'),
      where('patientId', '==', patient.id),
      where('docType', '==', 'evolution'),
      where('status', '==', 'signed')
    );
    const unsubscribe = onSnapshot(q, async (snap) => {
      try {
        ...
        for (const d of toMerge) {
          const data = d.data();
          const idx = data.evolutionIndex;
          const draft = (freshPatient.evolution || [])[idx];
          if (!draft) continue;
          const releasedAt = new Date().toISOString();
          const historyEntry = { ...draft, releasedAt, releasedBy: 'Paciente (assinatura remota)', signatureUrl: data.signatureUrl, sentVia: data.sentVia, sentTo: data.sentTo };
          const remainingDrafts = (freshPatient.evolution || []).filter((_, i) => i !== idx);
          await updateDoc(doc(db, 'patients', patient.id!), {
            evolution: remainingDrafts,
            evolutionHistory: [...(freshPatient.evolutionHistory || []), historyEntry],
            updatedAt: releasedAt,
          });
          await updateDoc(doc(db, 'signRequests', d.id), { mergedIntoRecord: true });
        }
        showToast('Registro de evolução assinado remotamente e liberado');
      } catch (err: any) {
        console.error('Erro ao mesclar assinatura remota de evolução:', err);
        showToast(`Não foi possível liberar a evolução: ${err?.code || err?.message || 'erro desconhecido'}`, 'error');
      }
    });
    return () => unsubscribe();
  }, [patient.id]);

  // Auto-save da evolução: mesmo motivo da anamnese — sem isso, se a pessoa saísse da
  // tela no meio do preenchimento (sem clicar em Salvar/Liberar), tudo que tinha
  // digitado se perdia. Só salva se tiver pelo menos algo digitado (evita criar
  // rascunhos vazios sozinho).
  const isFirstEvolutionRender = useRef(true);
  useEffect(() => {
    if (isFirstEvolutionRender.current) { isFirstEvolutionRender.current = false; return; }
    if (!isAddingEvolution) return;
    const hasContent = newEvolution.procedure.trim() || newEvolution.notes.trim() || newEvolution.bucoMaxiloNotes.trim() || newEvolution.numericValue.trim();
    ...
    const timeout = setTimeout(async () => {
      try {
        ...
        if (editingEvolutionIndex !== null) {
          ...
          updated[editingEvolutionIndex] = { ...updated[editingEvolutionIndex], ...entryData };
          await updateDoc(doc(db, 'patients', patient.id!), { evolution: updated, updatedAt: new Date().toISOString() });
        } else {
          const entry = { ...entryData, date: new Date().toISOString(), released: false };
          const updated = [entry, ...(patient.evolution || [])];
          await updateDoc(doc(db, 'patients', patient.id!), { evolution: updated, updatedAt: new Date().toISOString() });
          // Depois do primeiro auto-save, passa a ser "edição" do rascunho recém-criado —
          // sem isso, cada auto-save seguinte criaria um rascunho novo em vez de atualizar
          setEditingEvolutionIndex(0);
        }
      } catch { /* melhor esforço — o botão Salvar continua disponível como reforço */ }
    }, 2500);
    return () => clearTimeout(timeout);
  }, [newEvolution, isAddingEvolution]);
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                      ...
                      <div>
                        ...
                        <select
                          ...
                          disabled={!!patient.sex}
                          className="w-full bg-[#FDFBF9] border border-[#F5F2F0] rounded-2xl p-4 outline-none focus:border-[#EADFD4]/30 transition-all text-sm text-[#4A433D] disabled:opacity-60 disabled:cursor-not-allowed"
                        >
                          <option value="">—</option>
                        ...
                        {!!patient.sex && (
                          <p className="text-[10px] text-[#9CA3AF] mt-2 ml-1">
                            Não pode ser alterado depois de definido — fale com um administrador se precisar corrigir.
                          </p>
                        )}
                      </div>
                              ...
                              return (
                                ...
                                <div className="flex items-center gap-1">
                                  <button
                                    onClick={() => {
                                      if (active) {
                                        setAnamnesis(prev => ({ ...prev, plannedProcedures: (prev.plannedProcedures || []).filter(n => n !== proc.name) }));
                                        if (launched) handleUnlaunchFromFinance(proc.name);
                                        return;
                                      }
                                      ...
                                      setAnamnesis(prev => ({ ...prev, plannedProcedures: [...(prev.plannedProcedures || []), proc.name] }));
                                    ...
                                    <>
                                      <input
                                        ...
                                        onChange={e => {
                                          ...
                                          const updatedQty = { ...(anamnesis.plannedProcedureQuantities || {}), [proc.name]: q };
                                          setAnamnesis({ ...anamnesis, plannedProcedureQuantities: updatedQty });
                                          if (launched) handleUpdateLaunchedQuantity(proc.name, q);
                                        }}
                                        title="Quantidade de vezes"
                                        className="w-12 text-xs text-center border border-[#F5F2F0] rounded-xl py-2"
                                      />
                                      {launched && (
                    ...
                    >
                      ...
                      {!entry._released && entry._draftIndex !== -1 && (
                        <div className="flex gap-3 mt-6 pt-6 border-t border-[#F5F2F0]">
                          ...
                          <button
                            onClick={() => handleSendEvolutionForSignature(entry._draftIndex, 'whatsapp')}
                            disabled={sendingEvolutionSignIndex === entry._draftIndex}
                            className="flex items-center gap-2 text-[#9CA3AF] hover:text-[#4A433D] text-[10px] font-bold uppercase tracking-widest transition-all disabled:opacity-50"
                          >
                            <MessageCircle size={13} /> Assinatura Remota
                          </button>
                        </div>
                      )}
                    </motion.div>
          ...
          <div key={`budget-${i}`} className="p-8 bg-white border border-[#F5F2F0] rounded-[32px] space-y-6 shadow-sm hover:border-[#EADFD4]/30 transition-all">
            <div>
              <h4 className="text-lg font-normal text-[#4A433D] serif leading-tight">
                {b.budgetNumber ? `Orçamento nº ${b.budgetNumber} — ` : 'Orçamento — '}R$ {b.total.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}
              </h4>
              <p className="text-[9px] text-[#9CA3AF] font-bold uppercase tracking-[0.2em] mt-2">
