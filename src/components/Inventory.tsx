...
export default function Inventory({ user }: { user: User }) {
  ...
  const handleUpdateStock = async (item: InventoryItem, amount: number, type: 'consumption' | 'restock', spentValue?: number) => {
    ...
    try {
      // Custo por unidade recalculado a cada reposição com valor informado — reflete o
      // preço mais recente pago, usado no cálculo de lucro por procedimento.
      ...
      // Toda compra de material/insumo com valor informado já entra automaticamente
      // como despesa no Financeiro — sem isso, o dinheiro gasto reabastecendo o estoque
      // nunca aparecia no cálculo de entrada e saída da clínica.
      if (type === 'restock' && spentValue && spentValue > 0) {
        await addDoc(collection(db, 'transactions'), {
          ...
          date: new Date(),
          category: 'Insumos',
          description: `Compra de estoque — ${item.name} (${amount} ${item.unit.toLowerCase()})`,
          autoGenerated: true,
        });
      }
...
function RestockStockModal({ item, onRestock, onClose }: { item: InventoryItem; onRestock: (qty: number, spentValue?: number) => void; onClose: () => void }) {
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <FormField
            ...
            value={spentValue}
            onChange={setSpentValue}
            placeholder="R$"
          />
          <p className="text-[10px] text-[#9CA3AF] font-light -mt-3 ml-1">
            Se informado, entra automaticamente como despesa no Financeiro — sem isso, o
            gasto não aparece no cálculo de entrada e saída da clínica.
          </p>
          ...
          <div className="flex gap-4 pt-4">
            ...
            <button
              type="submit"
              className="flex-1 py-4 bg-[#8BA888] text-white rounded-2xl font-bold text-[10px] uppercase shadow-md hover:bg-[#7C9979] transition-all"
            >
              Registrar Reposição
            </button>
          </div>
...
function AddMaterialModal({ user, onClose }: any) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const parsedSpentValue = spentValue ? parseCurrencyInput(spentValue) : 0;
      // Custo por unidade = valor gasto ÷ quantidade comprada (sempre em unidades) —
      // usado depois pra calcular o lucro por procedimento na aba nova de Financeiro
      const lastUnitCost = parsedSpentValue > 0 && parsedQuantity > 0 ? parsedSpentValue / parsedQuantity : undefined;
      ...
      // Mesmo lançamento automático que já existe na Reposição de estoque — o valor
      // gasto na compra inicial de um material também precisa entrar como despesa no
      // Financeiro, senão o Balancete nunca vê esse gasto.
      if (parsedSpentValue > 0) {
        await addDoc(collection(db, 'transactions'), {
          userId: user.uid,
          amount: parsedSpentValue,
          type: 'expense',
          date: new Date(),
          category: 'Insumos',
          description: `Compra de estoque — ${name} (${parsedQuantity} ${unit.toLowerCase()})`,
          autoGenerated: true,
        });
      }
    ...
    <div className="fixed inset-0 bg-[#4A433D]/20 backdrop-blur-sm z-50 flex items-center justify-center p-6">
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div>
            <FormField label="Valor gasto na compra (opcional)" value={spentValue} onChange={setSpentValue} placeholder="R$" />
            <p className="text-[10px] text-[#9CA3AF] font-light mt-1.5 ml-1">
              Se informado, entra automaticamente como despesa no Financeiro — sem isso, o gasto não aparece no
              cálculo de entrada e saída da clínica.
            </p>
          </div>
