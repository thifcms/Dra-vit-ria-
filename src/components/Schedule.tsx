    ...
    try {
      ...
      if (status === 'completed' && appt && appt.value && !appt.financeGenerated) {
        await addDoc(collection(db, 'transactions'), {
          ...
          amount: appt.value,
          type: 'income',
          date: new Date(`${appt.date}T${appt.time}:00`),
          category: appt.notes || 'Procedimento',
          description: `${appt.notes || 'Procedimento'} — ${appt.patientName}`,
          patientId: appt.patientId,
          autoGenerated: true,
        });
        await updateDoc(doc(db, 'appointments', id), { financeGenerated: true });
        showToast('Status atualizado e lançamento financeiro criado');
      } else {
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          {!appointment && (
            <div className="p-6 bg-[#F8F9FA] rounded-3xl border border-[#F1F3F5] space-y-4">
              ...
              <select
                className="w-full bg-white border border-[#F1F3F5] rounded-2xl p-4 outline-none focus:border-[#D1C7BD]/30 transition-all font-light appearance-none text-sm"
                value={recurrence}
