// Versão visível no rodapé de Configurações — atualizada a cada publicação que muda
// algo perceptível. Serve como conferência rápida e sem dúvida: se o número aqui bate
// com o que a pessoa vê na tela, a versão certa carregou; se não bate, é cache/atualização
// pendente, não um bug no código novo.
export const APP_VERSION = '2026.08.04-1';
export const APP_VERSION_NOTE = 'Senha de Acesso movida pro Perfil, removida a duplicata da Gestão';
